\documentclass[aspectratio=169]{beamer}

\usetheme{Madrid}
\usecolortheme{default}
\usefonttheme{professionalfonts}

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[indonesian]{babel}
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage{graphicx}
\usepackage{array}
\title{Perancangan IP I2S TX AXI4-Lite pada SoC MicroBlaze V}
\subtitle{Implementasi pada FPGA Basys3 dengan DAC PCM5102A}
\author{Nama Mahasiswa \\ NIM}
\institute{Departemen Teknik Elektro dan Teknologi Informasi\\Universitas Gadjah Mada}
\date{April 2026}

\begin{document}

\begin{frame}
  \titlepage
\end{frame}

\begin{frame}{Outline Presentasi}
  \tableofcontents
\end{frame}

\section{Pendahuluan}

\begin{frame}{Latar Belakang}
  \begin{itemize}
    \item Kebutuhan sistem embedded audio meningkat, sehingga dibutuhkan antarmuka serial yang stabil dan terukur.
    \item I2S dipilih untuk jalur audio digital, sedangkan AXI4-Lite digunakan untuk kontrol register dari prosesor.
    \item Penelitian berfokus pada IP custom \textbf{I2S TX} berbasis Verilog pada SoC \textbf{MicroBlaze V}.
    \item Implementasi target: \textbf{Basys3} dengan keluaran ke DAC \textbf{PCM5102A}.
    \item Nilai tambah: pembuktian konsep \textit{hardware/software co-design} menggunakan antarmuka \textit{register-driven} murni.
  \end{itemize}
\end{frame}

\begin{frame}{Rumusan Masalah}
  \begin{enumerate}
    \item Bagaimana merancang IP \textbf{I2S TX} dengan antarmuka \textbf{AXI4-Lite} agar dapat diakses melalui register memory-mapped?
    \item Bagaimana mendukung dua mode frekuensi sampling (sekitar 48 kHz dan 96 kHz) dari satu \texttt{audio\_clk}?
    \item Bagaimana memverifikasi fungsionalitas dan timing melalui ILA, pengukuran frekuensi, serta uji audio PCM5102A?
  \end{enumerate}
\end{frame}

\begin{frame}{Tujuan dan Batasan}
  \textbf{Tujuan utama}
  \begin{itemize}
    \item Merancang IP I2S TX stereo 24-bit (slot 32-bit) dengan kontrol register AXI4-Lite.
    \item Mengintegrasikan IP ke Vivado Block Design dan mengoperasikan playback dari aplikasi Vitis.
    \item Memvalidasi sistem pada hardware nyata dengan ILA dan keluaran audio PCM5102A.
  \end{itemize}

  \vspace{0.5em}
  \textbf{Batasan penelitian}
  \begin{itemize}
    \item TX-only, format Philips I2S, bare-metal, tanpa DMA, tanpa FIFO, dan tanpa Linux.
    \item Platform uji: Basys3 + PCM5102A dengan metode \textit{programmed I/O} berbasis register murni.
  \end{itemize}
\end{frame}

\section{Dasar Teori}

\begin{frame}{Ringkasan Dasar Teori}
  \begin{itemize}
    \item \textbf{I2S}: tiga sinyal utama (BCLK, WS/LRCK, DATA), data serial MSB-first.
    \item \textbf{AXI4-Lite}: akses memory-mapped untuk konfigurasi dan pemantauan status peripheral.
    \item \textbf{Programmed I/O}: CPU mengendalikan pengiriman data sampel satu per satu ke register peripheral.
    \item \textbf{Underrun}: kondisi saat CPU terlambat mengirim data baru, menyebabkan putusnya suara.
    \item \textbf{Clocking MMCM}: nilai \textit{requested} dan \textit{actual} memengaruhi $F_s$ terukur.
  \end{itemize}
\end{frame}

\begin{frame}{Relasi Clock I2S}
  Untuk format stereo 32-bit per kanal:
  \[
    N_{bits/frame} = 64
  \]
  \[
    F_s = \frac{f_{BCLK}}{64}
  \]
  \[
    f_{BCLK} = \frac{f_{audio\_clk}}{\mathrm{divider} \times 2}
  \]

  \vspace{0.5em}
  \begin{itemize}
    \item \texttt{FS\_MODE=0}: divider 4 untuk target sekitar 48 kHz.
    \item \texttt{FS\_MODE=1}: divider 2 untuk target sekitar 96 kHz.
    \item Nilai aktual mengikuti hasil Clocking Wizard (requested vs actual).
  \end{itemize}
\end{frame}

\section{Perancangan Sistem}

\begin{frame}{Arsitektur Sistem}
  \begin{columns}[T]
    \column{0.58\textwidth}
      \begin{itemize}
        \item MicroBlaze V sebagai AXI master.
        \item AXI interconnect menuju IP I2S TX.
        \item Clocking Wizard menyediakan \texttt{audio\_clk}.
        \item Output I2S menuju PCM5102A.
      \end{itemize}
    \column{0.42\textwidth}
      \IfFileExists{contents/figures/block-diagram-sistem-i2s-axi.pdf}{%
        \includegraphics[width=\linewidth]{contents/figures/block-diagram-sistem-i2s-axi.pdf}%
      }{%
        \fbox{\parbox{0.95\linewidth}{\centering Placeholder gambar arsitektur sistem}}%
      }
  \end{columns}
\end{frame}

\begin{frame}{Desain Internal IP I2S TX}
  \begin{itemize}
    \item Blok utama: AXI register bank, clock divider, dan serializer I2S.
    \item Mekanisme push frame: firmware menulis \texttt{DATA\_LEFT} dan \texttt{DATA\_RIGHT}.
    \item Tidak ada buffer FIFO internal; sampel dikirim secara berkelanjutan lewat kontrol CPU (\textit{register-driven}).
  \end{itemize}

  \vspace{0.5em}
  \IfFileExists{contents/figures/block-diagram-ip-i2s-tx-intern.pdf}{%
    \centering\includegraphics[width=0.84\textwidth]{contents/figures/block-diagram-ip-i2s-tx-intern.pdf}%
  }{%
    \centering\fbox{\parbox{0.84\textwidth}{\centering Placeholder gambar blok internal IP}}
  }
\end{frame}

\begin{frame}{Ringkasan Register Map}
  \small
  \begin{tabular}{>{\ttfamily}p{1.3cm} p{1.2cm} p{7.2cm}}
    \toprule
    Offset & Akses & Fungsi \\
    \midrule
    0x00 & WO & DATA\_LEFT (24-bit data kanal kiri) \\
    0x04 & WO & DATA\_RIGHT (24-bit data kanal kanan) \\
    0x08 & RW & CONTROL: enable, mute, mode sampling, lebar sampel \\
    \bottomrule
  \end{tabular}
\end{frame}

\section{Implementasi dan Pengujian}

\begin{frame}{Implementasi di Vivado dan Vitis}
  \begin{itemize}
    \item RTL IP dikembangkan sebagai custom peripheral AXI4-Lite (register pembagi clock, serializer).
    \item Integrasi pada Vivado Block Design: MicroBlaze V, BRAM, Clocking Wizard, AXI interconnect, I2S IP.
    \item Alur aplikasi Vitis:
      \begin{itemize}
        \item inisialisasi register \texttt{CONTROL},
        \item pengiriman data berulang kali ke \texttt{DATA\_LEFT} dan \texttt{DATA\_RIGHT} lewat putaran waktu presisi (\textit{programmed I/O}).
      \end{itemize}
  \end{itemize}
\end{frame}

\begin{frame}{Metodologi Pengujian}
  \begin{itemize}
    \item Verifikasi bentuk sinyal I2S: BCLK, LRCK/WS, dan DATA pada satu frame stereo.
    \item ILA untuk observasi sinyal bus pada waktu nyata.
    \item Pengukuran frekuensi clock aktual dan estimasi $F_s$ untuk dua mode sampling.
    \item Uji audio pada PCM5102A menggunakan sinyal uji (misal sine 1 kHz).
  \end{itemize}

  \vspace{0.5em}
  \IfFileExists{contents/figures/ila-bclk-ws-data.png}{%
    \centering\includegraphics[width=0.80\textwidth]{contents/figures/ila-bclk-ws-data.png}%
  }{%
    \centering\fbox{\parbox{0.80\textwidth}{\centering Placeholder tangkapan ILA BCLK/WS/DATA}}
  }
\end{frame}

\begin{frame}{Ringkasan Hasil (Isi Nilai Pengukuran)}
  \small
  \begin{tabular}{lcc}
    \toprule
    Parameter & \texttt{FS\_MODE=0} & \texttt{FS\_MODE=1} \\
    \midrule
    $f_{audio\_clk}$ requested & 24.576 MHz & 24.576 MHz \\
    $f_{audio\_clk}$ aktual & 24.597 MHz & 24.597 MHz \\
    Divider yang dibutuhkan & 4 & 2 \\
    $f_{BCLK}$ hasil benar & 3.0746 MHz & 6.1493 MHz \\
    $F_s$ hasil benar & 48.03 kHz & 96.08 kHz \\
    Gejala RTL awal & benar & turun jadi 24.02 kHz \\
    Catatan & tervalidasi & perlu balik logika \texttt{FS\_MODE} \\
    \bottomrule
  \end{tabular}
\end{frame}

\begin{frame}{Keterkaitan Hasil terhadap Tujuan}
  \begin{itemize}
    \item \textbf{Tujuan 1 (IP AXI4-Lite)}: dibuktikan oleh register map dan operasi pembaruan register secara terus-menerus.
    \item \textbf{Tujuan 2 (dua mode sampling)}: analisis menunjukkan divider 4 dan 2 memang cukup untuk 48/96 kHz pada clock aktual 24.597 MHz, tetapi implementasi awal masih perlu koreksi pada logika \texttt{FS\_MODE}.
    \item \textbf{Tujuan 3 (validasi hardware)}: dibuktikan melalui pengujian ILA dan kualitas keluaran pada PCM5102A, sekaligus mengungkap isu integrasi clock family 44.1/48 kHz di top-level.
  \end{itemize}
\end{frame}

\section{Penutup}

\begin{frame}{Kesimpulan}
  \begin{itemize}
    \item IP I2S TX AXI4-Lite berbasis antarmuka register berhasil dirancang dan terintegrasi pada SoC MicroBlaze V di Basys3.
    \item Dua mode sampling berhasil diimplementasikan dari satu sumber \texttt{audio\_clk}.
    \item Menunjukkan konsep kontrol berbasis hardware vs software, di mana kesederhanaan hardware ditukar dengan beban CPU yang presisi.
    \item Validasi fungsional dan timing dilakukan melalui ILA serta uji keluaran PCM5102A.
  \end{itemize}
\end{frame}

\begin{frame}{Saran Pengembangan}
  \begin{itemize}
    \item Menambahkan antrean buffer memori FIFO pada perangkat keras.
    \item Integrasi DMA dan mekanisme interupsi watermark untuk mengurangi beban sinkronisasi CPU.
    \item Menambah variasi format audio dan frekuensi sampling.
  \end{itemize}
\end{frame}

\begin{frame}
  \centering
  \Large Terima kasih
\end{frame}

\end{document}
