...
\begin{itemize}
    \item Modul inti \path{i2s_slave_lite_v1_0_S00_AXI.v} membangkitkan \texttt{BCLK}, \texttt{WS}, dan \texttt{MCLK} menggunakan akumulator fase DDS pada domain \texttt{S\_AXI\_ACLK} 100 MHz.
    ...
    \item Wrapper \path{i2s.v} mempertahankan port \texttt{audio\_48\_clk}/\texttt{audio\_44\_clk} untuk kompatibilitas block design, meskipun logika DDS tidak menggunakannya.
    \item Kode lengkap berada di \path{latesti2s/ready_to_run/ip_repo/i2s_1_0/hdl}; firmware uji di \path{latesti2s/ready_to_run/sw/helloworld.c}.
\end{itemize}

Potongan kode DDS yang menunjukkan pembangkitan detak BCLK ditampilkan pada Listing~\ref{lst:app-dds-bclk}.

\begin{lstlisting}[language=Verilog, caption={Akumulator fase DDS untuk pembangkitan BCLK pada \texttt{i2s\_slave\_lite\_v1\_0\_S00\_AXI.v}.}, label={lst:app-dds-bclk}]
