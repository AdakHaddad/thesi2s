...
RTL direalisasikan dalam bahasa Verilog dengan slave AXI4-Lite, FIFO, unit interupsi, pembagi clock untuk dua mode \(F_s\), dan serializer Philips I2S 24-bit pada slot 32-bit. Integrasi dilakukan melalui IP Integrator; pin I2S dan opsional \texttt{irq} dipetakan pada file \texttt{.xdc}.
...
Jika testbench disimulasikan, lampirkan cuplikan gelombang verifikasi serializer dan FIFO (push/pop).

\section{Pengujian dengan ILA}

Probe minimal: \texttt{i2s\_bclk}, \texttt{i2s\_ws}, \texttt{i2s\_data}, level FIFO, \texttt{underrun}, \texttt{irq}. Trigger pada tepi WS memudahkan verifikasi 64 BCLK per frame.
...
Dokumentasikan \textit{pain points}: misalnya CDC register--FIFO, prioritas ISR, kesalahan constraint pin, atau deviasi MMCM; serta solusi yang diterapkan.

\section{Pembahasan terhadap tujuan penelitian}

\begin{enumerate}
	\item Tujuan perancangan IP: (\textit{jelaskan apakah register map, FIFO, irq, dan serializer telah terbukti pada hardware}).
	\item Tujuan integrasi SoC dan Vitis: (\textit{jelaskan alur prefill + ISR}).
