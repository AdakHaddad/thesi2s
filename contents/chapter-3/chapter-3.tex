...
Bagian ini merangkum \textbf{perancangan fungsional} IP yang menjadi inti skripsi. Detail RTL dan register final mengacu pada dokumentasi proyek FPGA (repositori implementasi).

\subsection{Daftar fitur IP}
\begin{itemize}
	\item Antarmuka \textbf{AXI4-Lite} (32 bit) untuk kontrol dan pengisian data audio.
	\item Mode \textbf{TX-only}; stereo; format \textbf{Philips I2S}; \textbf{24 bit} per saluran dalam \textbf{slot 32 bit}.
	\item Dua mode frekuensi sampling (nilai nominal \textbf{sekitar} 48 kHz dan 96 kHz) melalui pembagi \texttt{audio\_clk}.
	\item \textbf{FIFO TX} dengan status level, \textit{underrun}, dan \textbf{interupsi watermark} (serta enable/clear pada register).
	\item Keluaran ke modul \textbf{PCM5102A}: BCLK, LRCK/WS, DATA (dan MCLK bila diperlukan sesuai modul).
\end{itemize}
...
\begin{figure}[htbp]
	...
	\caption{Blok diagram internal IP I2S TX (konsep).}
	\label{fig:ch3-block-ip-intern}
\end{figure}

\subsection{Register map (target implementasi)}
Tabel \ref{tab:ch3-regmap} merangkum register word-aligned yang dipakai pada implementasi final. Fokus desain adalah tiga register utama untuk firmware: \texttt{DATA\_LEFT}, \texttt{DATA\_RIGHT}, dan \texttt{CONTROL}. Satu word tambahan \texttt{RESERVED} dipertahankan untuk ekspansi di masa depan.

\begin{table}[htbp]
\centering
\caption{Register map IP I2S AXI4-Lite (implementasi final).}
\label{tab:ch3-regmap}
\begin{tabular}{|l|l|p{6.3cm}|p{3.4cm}|}
\hline
	extbf{Offset} & \textbf{Nama Register} & \textbf{Fungsi} & \textbf{Packing / Catatan} \\
\hline
	exttt{0x00} & \texttt{DATA\_LEFT} & Sampel audio kanal kiri & \texttt{[SAMPLE\_WIDTH-1:0]} berisi data sampel kiri, sisanya 0 \\
	exttt{0x04} & \texttt{DATA\_RIGHT} & Sampel audio kanal kanan & \texttt{[SAMPLE\_WIDTH-1:0]} berisi data sampel kanan, sisanya 0 \\
	exttt{0x08} & \texttt{CONTROL} & Enable, mute, mode sampling, dan lebar sampel & Lihat Tabel \ref{tab:ch3-control-bits} \\
	exttt{0x0C} & \texttt{RESERVED} & Cadangan untuk ekspansi register di masa depan & Tidak digunakan pada versi ini \\
\hline
\end{tabular}
\end{table}
...
\begin{tabular}{|c|l|c|p{8.2cm}|}
\hline
	extbf{Bit(s)} & \textbf{Field} & \textbf{Default} & \textbf{Fungsi / Keterangan} \\
\hline
	exttt{0} & \texttt{ENABLE} & 0 & 1 = output aktif, 0 = output nonaktif \\
	exttt{1} & \texttt{MUTE} & 0 & 1 = data audio dipaksa nol (mute) \\
	exttt{2} & \texttt{FS\_MODE} & 0 & 0/1 = memilih mode frekuensi sampling keluarga cepat / lambat \\
	exttt{3} & \texttt{FS\_FAMILY} & 0 & 0 = keluarga 48/96 kHz; 1 = keluarga 44.1/88.2 kHz \\
	exttt{7:4} & \texttt{RESERVED} & 0 & Cadangan untuk ekstensi berikutnya \\
	exttt{12:8} & \texttt{SAMPLE\_WIDTH} & 24 & Lebar sampel aktual per kanal, rentang 1--32 bit \\
	exttt{31:13} & \texttt{RESERVED} & 0 & Cadangan untuk perluasan kontrol di masa depan \\
\hline
\end{tabular}
\end{table}

\noindent\textbf{Aturan packing data:}
\begin{itemize}
\item \texttt{DATA\_LEFT}: \texttt{[SAMPLE\_WIDTH-1:0]} berisi sampel kiri, \texttt{[31:SAMPLE\_WIDTH]} bernilai 0.
\item \texttt{DATA\_RIGHT}: \texttt{[SAMPLE\_WIDTH-1:0]} berisi sampel kanan, \texttt{[31:SAMPLE\_WIDTH]} bernilai 0.
\item Firmware menulis pasangan data kiri/kanan sebelum mengaktifkan \texttt{ENABLE}.
\item Jika \texttt{MUTE=1}, keluaran data dipaksa nol meskipun register data terisi.
\end{itemize}

\noindent\textbf{Contoh nilai \texttt{CONTROL}:}
\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
	extbf{ENABLE} & \textbf{MUTE} & \textbf{FS\_MODE} & \textbf{FS\_FAMILY} & \textbf{SAMPLE\_WIDTH} & \textbf{Makna} \\
\hline
1 & 0 & 0 & 0 & 24 & 96 kHz, 24 bit \\
1 & 0 & 1 & 0 & 16 & 48 kHz, 16 bit \\
1 & 1 & 1 & 1 & 24 & 44.1 kHz, mute \\
1 & 0 & 0 & 1 & 32 & 88.2 kHz, 32 bit \\
\hline
\end{tabular}
\end{center}
...
\begin{figure}[htbp]
	...
	\caption{Integrasi IP pada Block Design (MicroBlaze, interconnect, clock, I2S).}
