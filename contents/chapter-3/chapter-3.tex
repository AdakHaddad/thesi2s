...
Tabel \ref{tab:ch3-regmap} merangkum offset register word-aligned. Akses \textbf{W1C} = tulis-1-untuk-\textit{clear}; \textbf{W1P} = pulsa tulis satu siklus (sesuai implementasi).

\begin{table}[htbp]
	\centering
	\caption{Ringkasan register IP I2S TX (target).}
	\label{tab:ch3-regmap}
	\begin{tabular}{|l|l|p{6.5cm}|}
		\hline
		\textbf{Offset} & \textbf{Akses} & \textbf{Keterangan} \\
		\hline
		\texttt{0x00} & RO & \texttt{ID} / versi \\
		\texttt{0x04} & RW & \texttt{CONTROL}: enable, mute, \texttt{FS\_MODE}, reset FIFO, dsb. \\
		\texttt{0x08} & RO & \texttt{STATUS}: level FIFO, empty/full, underrun \\
		\texttt{0x0C} & RW & \texttt{WATERMARK} \\
		\texttt{0x10} & RW & \texttt{IRQ\_EN} \\
		\texttt{0x14} & RW1C & \texttt{IRQ\_STATUS} \\
		\texttt{0x1C} & WO & \texttt{TX\_LEFT} (24-bit) \\
		\texttt{0x20} & WO & \texttt{TX\_RIGHT}: tulis meng-\textit{push} satu frame stereo \\
		\hline
	\end{tabular}
\end{table}
